...
export function CategoriesAsyncSelect({
  selectedCategory,
  setSelectedCategory,
}: {
  selectedCategory: TrailCategory;
  setSelectedCategory: (category: TrailCategory) => void;
}) {
  ...
  return (
    <AsyncSelect<TrailCategory>
      ...
      renderOption={category => <div className="font-medium">{category.name}</div>}
      notFound={query =>
        query.trim().length > 3 ? (
          <div className="p-3 pb-1">
            <p className="mb-2 text-center text-sm text-gray-500">Nada encontrado para &quot;{query}&quot;</p>
            <Button
              className="w-full"
              variant="ghost"
              size="sm"
          ...
          <div className="p-3 pb-1">
            <p className="text-center text-sm text-gray-500">Pesquise algo!</p>
          </div>
        )
      }
