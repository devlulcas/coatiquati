...
export async function getReportsQuery(params: ReportSearchSchema = { skip: 0, take: 30 }): Promise<Report[]> {
  const session = await getPageSession();

  if (!isAdminOrAbove(session?.user.role)) {
    throw new Error('Você não tem permissão para ver os reports');
  }
