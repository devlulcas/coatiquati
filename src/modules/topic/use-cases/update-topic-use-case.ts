...
export async function updateTopicUseCase(params: UpdateTopicSchema): Promise<Topic> {
  ...
  try {
    return repository.updateTopic(validatedParams.data.topicId, validatedParams.data.topic);
  } catch (error) {
    console.error(error);
